        ...
        <AnimatePresence mode="popLayout">
          {featuredProjects.map((p, i) => (
            <motion.article
              ...
              className="glass corner-brackets group relative flex flex-col lg:flex-row overflow-hidden rounded-3xl p-6 lg:p-8 bg-black/20 lg:col-span-2"
            >
              <div
              ...
              <div className="flex-1 rounded-xl bg-black/50 border border-white/10 p-4 font-mono text-xs overflow-hidden flex flex-col shadow-inner mt-6 lg:mt-0 lg:ml-6">
      ...
      {otherProjects.length > 0 && (
        <div className="mt-16">
          <div className="flex items-center gap-4 mb-8">
            <h3 className="text-xl font-bold tracking-tight text-white">Other Explorations</h3>
          ...
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pb-8 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {otherProjects.map((p) => (
              <article
                key={p.id}
                className="snap-start shrink-0 w-[300px] sm:w-[350px] glass corner-brackets group relative flex flex-col overflow-hidden rounded-3xl p-6 transition-transform hover:-translate-y-2"
              >
                <div
                  className="pointer-events-none absolute -right-16 -top-16 h-32 w-32 rounded-full opacity-10 blur-2xl transition-opacity group-hover:opacity-30"
                  style={{ background: p.accent }}
                />
